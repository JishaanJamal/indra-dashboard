...
html,body{background:var(--bg);color:var(--text);font-family:var(--font);font-size:11px;min-height:100vh;width:100vw}
...
#dashboard{position:relative;width:100%;min-height:calc(100vh - 58px);overflow-y:auto;overflow-x:hidden;padding:10px;background:linear-gradient(rgba(0,212,170,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(0,212,170,0.03) 1px,transparent 1px);background-size:20px 20px}
...
<div id="header">
  ...
  <div class="status">
    <span id="conn"><span class="dot demo" id="dot"></span> <span id="mode">DEMO DATA</span></span>
...
<script>
const CONFIG={defaults:[{t:'ta',x:10,y:10,w:340,h:260},{t:'nifty15',x:360,y:10,w:520,h:260},{t:'volume',x:10,y:280,w:320,h:260},{t:'gainers',x:340,y:280,w:320,h:260},{t:'indices',x:670,y:10,w:440,h:140},{t:'heatmap',x:670,y:160,w:440,h:220},{t:'aapl',x:10,y:550,w:520,h:300},{t:'metals',x:540,y:550,w:280,h:180},{t:'clocks',x:540,y:740,w:280,h:180}],refresh:5000,live:false};
let W=[],nextId=1,dragW=null,resizeW=null,isDrag=false,isResize=false,dOff={x:0,y:0},rStart={x:0,y:0,w:0,h:0},liveData={},taData={},niftyCandles=[];
const dash=document.getElementById('dashboard'),tip=document.getElementById('tooltip');

// REALISTIC DEMO DATA (matches NSE India ~22,500 levels)
...
// INDICATORS
function sma(data,period){let r=[];for(let i=period-1;i<data.length;i++){let s=0;for(let j=0;j<period;j++)s+=data[i-j].c;r.push({d:data[i].date,v:s/period})}return r}
function ema(data,period){let k=2/(period+1),r=[],e=data.slice(0,period).reduce((s,d)=>s+d.c,0)/period;for(let i=period;i<data.length;i++){e=data[i].c*k+e*(1-k);r.push({d:data[i].date,v:e})}return r}
function rsi(data,period=14){let g=0,l=0,r=[];for(let i=1;i<=period;i++){let ch=data[i].c-data[i-1].c;if(ch>0)g+=ch;else l-=ch}let ag=g/period,al=l/period;for(let i=period+1;i<data.length;i++){let ch=data[i].c-data[i-1].c,cg=ch>0?ch:0,cl=ch<0?-ch:0;ag=(ag*(period-1)+cg)/period;al=(al*(period-1)+cl)/period;let rs=al===0?100:100-(100/(1+ag/al));r.push({d:data[i].date,v:rs})}return r}
function macd(data,fast=12,slow=26,signal=9){let ef=ema(data,fast),es=ema(data,slow),md=[],off=slow-fast;for(let i=0;i<es.length;i++)if(i+off<ef.length)md.push({d:es[i].d,v:ef[i+off].v-es[i].v});let sg=[],k=2/(signal+1),em=md.slice(0,signal).reduce((s,d)=>s+d.v,0)/signal;for(let i=signal;i<md.length;i++){em=md[i].v*k+em*(1-k);sg.push({d:md[i].d,v:em})}let hi=[];for(let i=0;i<sg.length;i++)if(i+signal<md.length)hi.push({d:sg[i].d,v:md[i+signal].v-sg[i].v});return{macd:md,sig:sg,hist:hi}}
function vwap(data){let cpv=0,cv=0;return data.map(d=>{let tp=(d.h+d.l+d.c)/3;cpv+=tp*d.v;cv+=d.v;return{d:d.date,v:cpv/cv}})}
function analyze(candles){
  if(candles.length<30)return{bias:'NEUTRAL',conf:0,score:0,sig:[],rsi:50,macd:0};
  let l=candles[candles.length-1],rsiV=rsi(candles,14),rsiL=rsiV.length>0?rsiV[rsiV.length-1].v:50;
  ...
  let sma20=sma(candles,20),smaL=sma20.length>0?sma20[sma20.length-1].v:l.c;
  let ema9=ema(candles,9),emaL=ema9.length>0?ema9[ema9.length-1].v:l.c;
  let score=0,sig=[];
  if(rsiL<30){score+=2;sig.push('RSI oversold')}else if(rsiL<40){score+=1;sig.push('RSI weak bullish')}else if(rsiL>70){score-=2;sig.push('RSI overbought')}else if(rsiL>60){score-=1;sig.push('RSI weak bearish')}
  if(macL>0&&macP<=0){score+=2;sig.push('MACD bullish cross')}else if(macL>0){score+=1;sig.push('MACD positive')}else if(macL<0&&macP>=0){score-=2;sig.push('MACD bearish cross')}else if(macL<0){score-=1;sig.push('MACD negative')}
  if(l.c>vwapL){score+=1;sig.push('Above VWAP')}else{score-=1;sig.push('Below VWAP')}
  if(l.c>smaL){score+=1;sig.push('Above SMA20')}else{score-=1;sig.push('Below SMA20')}
  if(emaL>smaL){score+=1;sig.push('EMA9>SMA20')}else{score-=1;sig.push('EMA9<SMA20')}
  if(l.c>l.o)score+=0.5;else score-=0.5;
  let conf=Math.min(Math.abs(score)/4.5*100,100);
  let bias='NEUTRAL';if(score>=2)bias='BULLISH';else if(score<=-2)bias='BEARISH';else if(score>0)bias='SLIGHTLY BULLISH';else if(score<0)bias='SLIGHTLY BEARISH';
...
const R={
  ...
  nifty15(w){let c=document.createElement('canvas');c.className='chart';w.body.innerHTML='';w.body.appendChild(c);w.canvas=c;let ro=new ResizeObserver(()=>{let d=niftyCandles.length&&CONFIG.live?niftyCandles:genCandles(60,22534);drawCandles(w,d,'NIFTY 50 — 15 Min',CONFIG.live?'Live Yahoo (15m delay)':'DEMO DATA — NOT REAL',CONFIG.live)});ro.observe(w.body);w._ro=ro;let d=niftyCandles.length&&CONFIG.live?niftyCandles:genCandles(60,22534);drawCandles(w,d,'NIFTY 50 — 15 Min',CONFIG.live?'Live Yahoo (15m delay)':'DEMO DATA — NOT REAL',CONFIG.live)},
  ta(w){
    let a=taData.analysis,c=taData.candles;
    if(!a||!c||!CONFIG.live){w.body.innerHTML='<div style="text-align:center;padding:30px;color:var(--dim)"><b>DEMO MODE</b><br><br>Data is simulated.<br>NIFTY 50 demo base: ~22,534<br><br><button onclick="toggleMode()" style="background:var(--green);color:var(--bg);border:none;padding:6px 16px;border-radius:3px;cursor:pointer;font-family:inherit;font-size:11px;font-weight:bold">SWITCH TO LIVE</button><br><br><span style="font-size:9px">Live mode uses Yahoo Finance (15-min delayed)</span></div>';return}
    let l=c[c.length-1];
    ...
    let rsiC=a.rsi>70?'var(--red)':a.rsi<30?'var(--green)':'var(--text)';h+=`<div style="background:rgba(0,0,0,0.2);border:1px solid var(--border);border-radius:3px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--dim)">RSI (14)</div><div style="font-size:18px;font-weight:bold;color:${rsiC}">${a.rsi.toFixed(1)}</div><div class="${a.rsi>70?'down':a.rsi<30?'up':'muted'}">${a.rsi>70?'SELL':a.rsi<30?'BUY':'NEUTRAL'}</div></div>`;
    h+=`<div style="background:rgba(0,0,0,0.2);border:1px solid var(--border);border-radius:3px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--dim)">MACD Hist</div><div style="font-size:18px;font-weight:bold;color:${a.macd>0?'var(--green)':'var(--red)'}">${a.macd>0?'+':''}${a.macd.toFixed(2)}</div><div class="${a.macd>0?'up':'down'}">${a.macd>0?'BULLISH':'BEARISH'}</div></div>`;
    let vwpD=((l.c-a.vwap)/a.vwap*100).toFixed(2);h+=`<div style="background:rgba(0,0,0,0.2);border:1px solid var(--border);border-radius:3px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--dim)">VWAP</div><div style="font-size:14px;font-weight:bold">${a.vwap.toFixed(2)}</div><div class="${vwpD>0?'up':'down'}">${vwpD>0?'ABOVE':'BELOW'}</div></div>`;
    let smaD=((l.c-a.sma20)/a.sma20*100).toFixed(2);h+=`<div style="background:rgba(0,0,0,0.2);border:1px solid var(--border);border-radius:3px;padding:8px;text-align:center"><div style="font-size:9px;color:var(--dim)">SMA20</div><div style="font-size:14px;font-weight:bold">${a.sma20.toFixed(2)}</div><div class="${smaD>0?'up':'down'}">${smaD>0?'ABOVE':'BELOW'}</div></div>`;
    h+=`</div>`;
    h+=`<div style="margin-top:6px;padding:6px;background:rgba(0,0,0,0.2);border:1px solid var(--border);border-radius:3px"><div style="font-size:9px;color:var(--dim);margin-bottom:4px">SIGNALS</div>`;a.sig.forEach(s=>h+=`<div style="font-size:10px;padding:1px 0">• ${s}</div>`);h+=`</div>`;
...
function drawCandles(w,data,title,sub,showLines){
  ...
  if(showLines&&data.length>20){let s20=sma(data,20);if(s20.length){ctx.strokeStyle='rgba(88,166,255,0.6)';ctx.lineWidth=1;ctx.beginPath();s20.forEach((p,i)=>{let x=pad.l+((data.length-s20.length+i)/(data.length-1))*cw,y=pad.t+((maxP-p.v)/range)*ch;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.stroke()}let vp=vwap(data);if(vp.length){ctx.strokeStyle='rgba(188,140,255,0.6)';ctx.lineWidth=1;ctx.setLineDash([4,4]);ctx.beginPath();vp.forEach((p,i)=>{let x=pad.l+(i/(data.length-1))*cw,y=pad.t+((maxP-p.v)/range)*ch;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.stroke();ctx.setLineDash([])}}
...
// LIVE MODE (Yahoo Finance - 15 min delayed)
async function fetchYahooChart(sym,intv='15m',rng='5d'){try{let u=`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(sym)}?interval=${intv}&range=${rng}`;let r=await fetch(u,{cache:'no-store'});if(!r.ok)return null;let j=await r.json();if(!j.chart||!j.chart.result||!j.chart.result[0])return null;let res=j.chart.result[0],ts=res.timestamp,q=res.indicators.quote[0];let out=[];for(let i=0;i<ts.length;i++)if(q.open[i]!=null)out.push({o:q.open[i],h:q.high[i],l:q.low[i],c:q.close[i],v:q.volume[i]||0,date:new Date(ts[i]*1000)});return out}catch(e){return null}}
async function updateLive(){if(!CONFIG.live)return;try{let nc=await fetchYahooChart('^NSEI','15m','5d');if(nc&&nc.length>0){niftyCandles=nc;taData.candles=nc;taData.analysis=analyze(nc)}document.getElementById('dot').className='dot live';document.getElementById('mode').textContent='LIVE (15m delay)';updateSentiment();W.forEach(w=>refresh(w.id))}catch(e){console.error('Live error:',e);document.getElementById('dot').className='dot demo';document.getElementById('mode').textContent='API ERROR'}}
function updateSentiment(){let a=taData.analysis,c=taData.candles;if(!a||!c)return;let l=c[c.length-1];document.getElementById('s-nifty').textContent=l.c.toFixed(2);document.getElementById('s-bias').textContent=`${a.bias} (${a.conf}%)`;document.getElementById('s-bias').style.color=a.bias.includes('BULL')?'var(--green)':a.bias.includes('BEAR')?'var(--red)':'var(--yellow)';document.getElementById('sentiment').className=a.bias.includes('BULL')?'bullish':a.bias.includes('BEAR')?'bearish':'neutral';document.getElementById('s-rsi').textContent=a.rsi.toFixed(1);document.getElementById('s-macd').textContent=(a.macd>0?'+':'')+a.macd.toFixed(2)}
function toggleMode(){CONFIG.live=!CONFIG.live;let btn=document.querySelector('[onclick="toggleMode()"]');if(CONFIG.live){btn.textContent='DEMO';btn.style.color='var(--red)';updateLive();setInterval(updateLive,30000)}else{btn.textContent='LIVE';btn.style.color='var(--green)';document.getElementById('dot').className='dot demo';document.getElementById('mode').textContent='DEMO DATA';document.getElementById('s-nifty').textContent='--';document.getElementById('s-bias').textContent='NEUTRAL';document.getElementById('s-bias').style.color='';document.getElementById('sentiment').className='neutral';document.getElementById('s-rsi').textContent='--';document.getElementById('s-macd').textContent='--';niftyCandles=[];taData={};W.forEach(w=>refresh(w.id))}}
...
function init(){try{if(!load())CONFIG.defaults.forEach(d=>create(d.t,d.x,d.y,d.w,d.h));setInterval(()=>{if(!CONFIG.live)W.forEach(w=>{if(w.t!=='aapl'&&w.t!=='nifty15'&&w.t!=='clocks'&&w.t!=='ta')refresh(w.id)})},CONFIG.refresh);setInterval(()=>W.filter(w=>w.t==='clocks').forEach(w=>updateClocks(w)),1000)}catch(e){console.error('Init error:',e);alert('Dashboard loaded with errors. Check console (F12).')}}
