...
body{background:var(--bg);color:var(--text);font-family:monospace;font-size:11px;min-height:100vh;overflow-y:auto}
...
th{text-align:left;padding:3px 5px;color:var(--dim);border-bottom:1px solid var(--border);font-size:9px;text-transform:uppercase;background:var(--panel)}
...
.heat-cell{display:flex;flex-direction:column;align-items:center;padding:6px;border-radius:2px;font-size:9px;text-align:center;background:rgba(0,0,0,0.2);border:1px solid var(--border)}
...
#add-menu{position:fixed;top:36px;right:8px;background:var(--panel);border:1px solid var(--border);border-radius:4px;padding:6px;z-index:10001;display:none;min-width:180px;box-shadow:0 8px 30px rgba(0,0,0,0.8)}
...
<div id="header">
  ...
  <div style="display:flex;align-items:center;gap:8px">
    <span id="clock">--:--:--</span>
    <button onclick="toggleMenu()" style="background:#0a0a0f;border:1px solid var(--border);color:var(--green);padding:2px 8px;cursor:pointer;font-family:inherit;font-size:10px;border-radius:3px">+</button>
  </div>
</div>

<div id="sentiment">
  <span>NIFTY: <b>22,534.50</b></span>
  <span>BIAS: <b>NEUTRAL</b></span>
  <span>RSI: <b>--</b></span>
</div>
...
function toggleMenu(){document.getElementById('add-menu').classList.toggle('show')}
document.addEventListener('click',e=>{if(!e.target.closest('#add-menu')&&!e.target.closest('[onclick="toggleMenu()"]'))document.getElementById('add-menu').classList.remove('show')});
...
function addW(type){
  ...
  el.innerHTML=`<div class="widget-header"><span>${titles[type]||type.toUpperCase()}</span><div><button class="w-btn" onclick="refreshW('${id}','${type}')">↻</button><button class="w-btn" onclick="document.getElementById('${id}').remove()">×</button></div></div><div class="widget-body" id="b-${id}"></div>`;
  document.getElementById('dashboard').appendChild(el);
  refreshW(id,type);
  toggleMenu();
}

function refreshW(id,type){
  const b=document.getElementById('b-'+id);if(!b)return;
  if(type==='volume'){let d=stocks.map(s=>({s:s.s,n:s.n,p:r(s.p,0.01),v:Math.floor(s.v*(0.8+Math.random()*0.4)),c:(Math.random()-0.5)*4})).sort((a,b)=>b.v-a.v).slice(0,10);let h='<table><thead><tr><th>Sym</th><th>Price</th><th>Vol</th><th>Chg%</th></tr></thead><tbody>';d.forEach(x=>{let cl=x.c>=0?'up':'down',sg=x.c>=0?'+':'';h+=`<tr><td><b>${x.s}</b><br><span class="muted">${x.n}</span></td><td>₹${x.p.toFixed(2)}</td><td>${(x.v/1e6).toFixed(2)}M</td><td class="${cl}">${sg}${x.c.toFixed(2)}%</td></tr>`});b.innerHTML=h+'</tbody></table>';}
  else if(type==='gainers'){let d=stocks.map(s=>({s:s.s,n:s.n,p:r(s.p,0.01),c:(Math.random()-0.3)*5,v:Math.floor(s.v*(0.5+Math.random()))})).sort((a,b)=>b.c-a.c).slice(0,10);let h='<table><thead><tr><th>Sym</th><th>Price</th><th>Chg%</th><th>Vol</th></tr></thead><tbody>';d.forEach(x=>{let cl=x.c>=0?'up':'down',sg=x.c>=0?'+':'';h+=`<tr><td><b>${x.s}</b><br><span class="muted">${x.n}</span></td><td>₹${x.p.toFixed(2)}</td><td class="${cl}">${sg}${x.c.toFixed(2)}%</td><td>${(x.v/1e6).toFixed(2)}M</td></tr>`});b.innerHTML=h+'</tbody></table>';}
  else if(type==='indices'){let d=indices.map(i=>({n:i.n,v:r(i.v,0.005),c:i.c+(Math.random()-0.5)*0.3}));let h='<div style="display:flex;gap:6px;flex-wrap:wrap">';d.forEach(x=>{let cl=x.c>=0?'up':'down',sg=x.c>=0?'+':'';h+=`<div class="pill"><div class="pill-name">${x.n}</div><div class="pill-val">${x.v.toLocaleString('en-IN',{maximumFractionDigits:2})}</div><div class="pill-chg ${cl}">${sg}${x.c.toFixed(2)}%</div></div>`});b.innerHTML=h+'</div>';}
  else if(type==='heatmap'){let d=sectors.map(s=>({...s,c:s.c+(Math.random()-0.5)*1.5}));let h='<div class="heat">';d.forEach(x=>{let int=Math.min(Math.abs(x.c)/3,1),bg=x.c>=0?`rgba(0,${Math.floor(212*int)},${Math.floor(170*int)},${0.15+int*0.25})`:`rgba(${Math.floor(255*int)},${Math.floor(85*int)},${Math.floor(85*int)},${0.15+int*0.25})`;h+=`<div class="heat-cell" style="background:${bg};border:1px solid ${x.c>=0?'rgba(0,212,170,0.3)':'rgba(255,85,85,0.3)'}"><b>${x.sym}</b><span class="${x.c>=0?'up':'down'}">${x.c>=0?'+':''}${x.c.toFixed(2)}%</span></div>`});b.innerHTML=h+'</div>';}
  else if(type==='metals'){let d=metals.map(m=>({...m,p:r(m.p,0.008),c:m.c+(Math.random()-0.5)*0.5}));let h='';d.forEach(x=>{let cl=x.c>=0?'up':'down',sg=x.c>=0?'+':'';h+=`<div class="metal"><div><div class="muted">${x.n} (${x.sym})</div><div style="font-size:14px;font-weight:bold">$${x.p.toFixed(2)}</div><div class="${cl}">${sg}${x.c.toFixed(2)}%</div></div></div>`});b.innerHTML=h;}
  else if(type==='clocks'){b.innerHTML='<div class="clocks" id="cg-'+id+'"></div>';updateClocks(id);}
  else if(type==='aapl'){b.innerHTML='<canvas id="c-'+id+'" width="400" height="180"></canvas>';setTimeout(()=>drawChart('c-'+id,175,20,'AAPL — 60 Sessions'),50);}
  else if(type==='nifty15'){b.innerHTML='<canvas id="c-'+id+'" width="400" height="180"></canvas>';setTimeout(()=>drawChart('c-'+id,22500,200,'NIFTY 50 — 15 Min'),50);}
  else if(type==='ta'){b.innerHTML='<div style="text-align:center;padding:20px;color:var(--dim)"><b>TECHNICAL ANALYSIS</b><br><br>NIFTY 50 Base: 22,534.50<br><br>Demo mode — use broker API for live data.</div>';}
}

function updateClocks(id){
  const el=document.getElementById('cg-'+id);if(!el)return;
  const m=[{c:'Tokyo',tz:'Asia/Tokyo',o:9,cl:15},{c:'Mumbai',tz:'Asia/Kolkata',o:9,cl:15.5},{c:'London',tz:'Europe/London',o:8,cl:16.5},{c:'New York',tz:'America/New_York',o:9.5,cl:16}];
  let h='';m.forEach(x=>{let n=new Date(),ts=n.toLocaleTimeString('en-US',{timeZone:x.tz,hour12:false,hour:'2-digit',minute:'2-digit'}),hr=parseInt(n.toLocaleTimeString('en-US',{timeZone:x.tz,hour12:false,hour:'numeric'})),mn=parseInt(n.toLocaleTimeString('en-US',{timeZone:x.tz,hour12:false,minute:'numeric'})),dt=hr+mn/60,st='closed',txt='CLOSED';if(dt>=x.o&&dt<x.cl){st='open';txt='OPEN'}else if(Math.abs(dt-x.o)<0.5){st='pre';txt='PRE-OPEN'}h+=`<div class="clock"><div class="clock-city">${x.c}</div><div class="clock-time">${ts}</div><div class="clock-status ${st}">${txt}</div></div>`});el.innerHTML=h;
}

function drawChart(cid,base,scale,title){
  const c=document.getElementById(cid);if(!c)return;
  const ctx=c.getContext('2d'),w=c.width,h=c.height;
  ctx.fillStyle='#111118';ctx.fillRect(0,0,w,h);
  ctx.strokeStyle='rgba(31,31,46,0.6)';ctx.lineWidth=0.5;
  for(let i=0;i<=4;i++){let y=20+(h-40)/4*i;ctx.beginPath();ctx.moveTo(30,y);ctx.lineTo(w-50,y);ctx.stroke();ctx.fillStyle='#6e7681';ctx.font='8px monospace';ctx.textAlign='right';ctx.fillText((base+scale-(i*scale/4)).toFixed(0),w-5,y+3)}
  let p=base;const bw=5;
  for(let i=0;i<40;i++){let o=p,c=p+(Math.random()-0.48)*scale/10,hg=Math.max(o,c)+Math.random()*scale/20,l=Math.min(o,c)-Math.random()*scale/20,up=c>=o;ctx.strokeStyle=up?'#00d4aa':'#ff5555';ctx.fillStyle=up?'#00d4aa':'#ff5555';ctx.beginPath();ctx.moveTo(35+i*bw+2,20+(base+scale-hg)/scale*(h-40));ctx.lineTo(35+i*bw+2,20+(base+scale-l)/scale*(h-40));ctx.stroke();ctx.fillRect(35+i*bw,20+(base+scale-Math.max(o,c))/scale*(h-40),4,Math.max(Math.abs((c-o)/scale*(h-40)),1));p=c;}
  ctx.fillStyle='#00d4aa';ctx.font='bold 10px monospace';ctx.fillText(title,8,14);ctx.fillStyle='#6e7681';ctx.font='8px monospace';ctx.fillText('Demo Data',8,24);
}

setInterval(()=>document.getElementById('clock').textContent=new Date().toLocaleTimeString('en-IN',{hour12:false}),1000);
setInterval(()=>document.querySelectorAll('[id^="cg-"]').forEach(el=>{const id=el.id.replace('cg-','');updateClocks(id)}),1000);

// Add default widgets
['volume','gainers','indices','heatmap','nifty15','aapl','metals','clocks','ta'].forEach(t=>addW(t));
</script>
</body>
</html>
